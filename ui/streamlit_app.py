...
Single-file Streamlit app. Dark theme, tabbed navigation, no API calls
(playground uses illustrative sample data, not live retrieval) so it has
zero external dependencies to hang on.
"""
...
with tab_about:
    ...
    for category, skills in skill_categories.items():
        ...
        for col, skill in zip(cols, skills):
            with col:
                st.button(skill, disabled=True, use_container_width=True, key=f"skill_{category}_{skill}")
...
with tab_playground:
    ...
    pg_docs, pg_chat, pg_prompt, pg_format = st.tabs(
    ...
    with pg_chat:
        ...
        if query:
            ...
            st.markdown("---")
            st.info("🏆 **Best strategy: Hybrid** — Combines all signals for best results")
